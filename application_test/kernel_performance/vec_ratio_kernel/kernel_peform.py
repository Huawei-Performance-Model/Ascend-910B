'''
需要找到一种评判标准来表示kernel建模的精度
首先这里还是需要读取数据库文件的
因此首先参考数据集处理程序：
但是好像又不太需要，因为最终的数据集我们已经处理成.csv文件了
因此只要对这个.csv文件做一遍推理就可以了！！！！！！！
'''
import os
import time
import sqlite3
...
#暂时的目标是把单个算子的时间建模精度给量化出来
#首先这个程序不需要Graph,也不需要framework，aicore这些东西
#只需要读取对应size的csv文件,逐个推理
file_size={}
file_size[3]="../../../vec_ratio_size3/all_norm_tasktime.csv"
file_size[4]="../../../vec_ratio_size4/all_norm_tasktime.csv"
file_size[5]="../../../vec_ratio_size5/all_norm_tasktime.csv"
model={}#这里就不讲究了，直接手动挑选负载GPU
model[3]=torch.load("../../../vec_ratio_size3/size3_best.pth",map_location=device)
model[4]=torch.load("../../../vec_ratio_size4/size4_best.pth",map_location=device)
model[5]=torch.load("../../../vec_ratio_size5/size5_best.pth",map_location=device)
operator_cnt3=0
operator_cnt4=0
operator_cnt5=0
loss3=0
